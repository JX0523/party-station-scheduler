...
console.log('\n📋 场景2: 5部员+13部长(18人)，部员≥5→只用部员')
{
  ...
  assert('只用部员', r1.meta.roleLabel === '仅部员')
  assert('Week1 部长=0', r1.meta.perRoleUsed['部长'] === 0, `用了${r1.meta.perRoleUsed['部长']}个部长`)
  assert('Week2 部长=0', r2.meta.perRoleUsed['部长'] === 0, `用了${r2.meta.perRoleUsed['部长']}个部长`)
  assert('部员上限≤3', r1.meta.perRoleUsed['部员'] <= 3 && r2.meta.perRoleUsed['部员'] <= 3)
  const diff2 = Math.abs(r1.assignments.length - r2.assignments.length)
  assert('两周人数均衡(差≤1)', diff2 <= 1, `差${diff2}`)
}
...
console.log('\n📋 场景3: 4部员+8部长(12人)，部员<5→部员+部长')
{
  ...
  const diff = Math.abs(r1.assignments.length - r2.assignments.length)
  assert('两周人数差≤2', diff <= 2, `差${diff}`)
  assert('部员+部长模式', r1.meta.roleLabel === '部员+部长')
  assert('每周部员上限≤2(ceil(4/2))', r1.meta.perRoleMax['部员'] <= 2)
  assert('每周部长上限≤4(ceil(8/2))', r1.meta.perRoleMax['部长'] <= 4)
}
...
console.log('\n📋 场景5: 3部员+2部长+3主席团(8人)，部员+部长≥5→不启用主席团')
{
  ...
  assert('部员+部长层级', r1.meta.roleLabel === '部员+部长')
